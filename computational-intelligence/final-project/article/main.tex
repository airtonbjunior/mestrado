...
Os modelos criados após o processamento são apresentados em forma de árvores nas imagens \ref{AModel}, \ref{BModel} e \ref{CModel} (a variável x representa a entrada, nesse caso, os \emph{Tweets}). A tabela \ref{modelsExpressions} apresenta os modelos gerados em forma de expressões.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.35\textwidth]{treeC2}
		\caption{Modelo C}
		\label{CModel}
	\end{figure}
...
\centering
\begin{tabular}{cl}
\multicolumn{1}{c}{\textbf{Modelo}} & \multicolumn{1}{c}{\textbf{Expressão}} \\ \hline
A & \begin{tabular}[c]{@{}l@{}}if\_then\_else(hasEmoticons(x), emoticonsPolaritySum(x), polaritySum(x))\end{tabular} \\ \hline
B & \begin{tabular}[c]{@{}l@{}}mul(add(add(polaritySum(x),positiveEmoticons(x), mul(sub(sin(-0.7), div\\(negativeEmoticons(x), sqrt(div(log(0.30), cos(0.3))))), div(sin(negativeWords(x),\\ add(sqrt(cos(mul(hashtagPolaritySum(x),-1.1))), -0.2)))), mul(div(log (-0.9),\\ negativeEmoticons(x)), exp(add(-0.2, -0.2))))\end{tabular} \\ \hline
C & \begin{tabular}[c]{@{}l@{}}add(invertSignal(negativeWords(x)), sin(polaritySum(x)))\end{tabular} \\ \hline
\end{tabular}
\caption{Modelos gerados pela Programação Genética}
\label{modelsExpressions}
\end{table}
...
\centering
\begin{tabular}{lcccc}
\multicolumn{1}{c}{\textbf{Base}} & \multicolumn{1}{c}{\textbf{Melhor F1 médio (+/-)}} & \multicolumn{1}{c}{\textbf{Posição SemEval 2014}} & \multicolumn{1}{c}{\textbf{Top 3 SemEval}} \\ \hline
Tweets2013 &  0.5193 & 43º & \begin{tabular}[c]{@{}l@{}}1º 0.7212\\ 2º 0.7075\\ 3º 0.7040\end{tabular} \\ \hline
Tweets2014 &  0.458 & 48º & \begin{tabular}[c]{@{}l@{}}1º 0.7096\\ 2º 0.7014\\ 3º 0.6995\end{tabular} \\ \hline
TwitterSarcasm &  0.2854 & 52º & \begin{tabular}[c]{@{}l@{}}1º 0.5816\\ 2º 0.5726\\ 3º 0.5650\end{tabular} \\ \hline
SMS2013 & 0.4549 & 45º & \begin{tabular}[c]{@{}l@{}}1º 0.7028\\ 2º 0.6768\\ 3º 0.6751\end{tabular} \\ \hline
LiveJournal & 0.6052 & 37º & \begin{tabular}[c]{@{}l@{}}1º 0.7484\\ 2º 0.7446\\ 3º 0.7399\end{tabular} \\ \hline
\end{tabular}
\caption{Comparação de resultados com o SemEval 2014 \emph{task}9}
\label{compareResults}
\end{table}

\section{Considerações}\label{conclusion}

Como podemos perceber, os resultados do melhor classificador para a base \emph{TwitterSarcasm}, que possui sarcasmo em seu conteúdo, tiveram um resultado consideravelmente baixo - F1 médio (positivo e negativo) de 0.2854. Isso acontece pela dificuldade de identificação dessa figura de linguagem. A inclusão de novas funções e a utilização de bases de treinamento com mais frases contendo sarcasmo certamente trarão possibilidades de criação de modelos melhores para esse contexto.
